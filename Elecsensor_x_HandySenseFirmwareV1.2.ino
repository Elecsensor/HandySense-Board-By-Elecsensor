...
#define connect_WifiStatusToBox 33

/* new PCB Red */
int relay_pin[4] = { 14, 12, 13, 15 };
#define status_sht31_error 5
#define status_max44009_error 19
...
void UpdateData_To_Server() {
  ...
  if (check_sendData_toWeb == 1) {
    DatatoWeb = "{\"data\": {\"temperature\":" + String(temp) + ",\"humidity\":" + String(humidity) + ",\"lux\":" + String(lux_44009) + ",\"soil\":" + String(soil) + "}}";

    //digitalWrite(LEDY, HIGH);
    DEBUG_PRINT("DatatoWeb : ");
    DEBUG_PRINTLN(DatatoWeb);
    DatatoWeb.toCharArray(msgtoWeb, (DatatoWeb.length() + 1));
    if (client.publish("@shadow/data/update", msgtoWeb)) {
      check_sendData_toWeb = 0;
      DEBUG_PRINTLN("Send Data FULL");
    }
  }
}
...
void timmer_setting(String topic, byte* payload, unsigned int length) {
  ...
  if (state_On_Off_relay == 1) {
    for (int k = 0; k < 7; k++) {
      ...
      EEPROM.commit();
      DEBUG_PRINT("time_open  : ");
      DEBUG_PRINTLN(time_open[relay][k][timer]);
      DEBUG_PRINT("time_close : ");
      DEBUG_PRINTLN(time_close[relay][k][timer]);
    }
...
void SoilMaxMin_setting(String topic, String message, unsigned int length) {
  ...
  if (soil_topic.substring(9, 12) == "max") {
    ...
    DEBUG_PRINT("Max_Soil : ");
    DEBUG_PRINTLN(Max_Soil[Relay_SoilMaxMin]);
...
void TempMaxMin_setting(String topic, String message, unsigned int length) {
  ...
  if (temp_topic.substring(9, 12) == "max") {
    ...
    DEBUG_PRINT("Max_Temp : ");
    DEBUG_PRINTLN(Max_Temp[Relay_TempMaxMin]);
...
void Get_max44009() {
  ...
  //buffer_lux = lightMeter.readLightLevel() / 1000; //(KLux)
...
void setAll_config() {
  for (int b = 0; b < 4; b++) {
    ...
    DEBUG_PRINT(b);
    DEBUG_PRINT(" : ");
    DEBUG_PRINTLN(Max_Soil[b]);
    DEBUG_PRINT("Min_Soil   ");
    DEBUG_PRINT(b);
    DEBUG_PRINT(" : ");
    DEBUG_PRINTLN(Min_Soil[b]);
    DEBUG_PRINT("Max_Temp   ");
    DEBUG_PRINT(b);
    DEBUG_PRINT(" : ");
    DEBUG_PRINTLN(Max_Temp[b]);
    DEBUG_PRINT("Min_Temp   ");
    DEBUG_PRINT(b);
    DEBUG_PRINT(" : ");
    DEBUG_PRINTLN(Min_Temp[b]);
  }
  int count_in = 0;
  for (int eeprom_relay = 0; eeprom_relay < 4; eeprom_relay++) {
    for (int eeprom_timer = 0; eeprom_timer < 3; eeprom_timer++) {
      for (int dayinweek = 0; dayinweek < 7; dayinweek++) {
        ...
        DEBUG_PRINT("cout       : ");
        DEBUG_PRINTLN(count_in);
        DEBUG_PRINT("time_open  : ");
        DEBUG_PRINTLN(time_open[eeprom_relay][dayinweek][eeprom_timer]);
        DEBUG_PRINT("time_close : ");
        DEBUG_PRINTLN(time_close[eeprom_relay][dayinweek][eeprom_timer]);
        count_in++;
      }
    }
  }
...
void IRAM_ATTR Blink_LED() {
  if (connectWifiStatus == editDeviceWifi) {
    ...
    if (buff_count_LED_serverConnected < 7) {
      ...
      buff_count_LED_serverConnected = 0;
  }
}
